...
client.on('ready', async () => {
    ...
    console.log(
        `Total hours: ${report.summary.totalHours}h (${report.summary.totalMinutes}min)`
    );
    ...
    report.absences.slice(0, 3).forEach(a => {
        console.log(
            `  ${a.startDate.toLocaleDateString()} - ${a.durationHours}h`
        );
    ...
    report.lates.slice(0, 3).forEach(l => {
        console.log(
            `  ${l.startDate.toLocaleDateString()} - ${l.durationMinutes}min`
        );
    ...
    if (report.earlyDepartures.length > 0) {
        report.earlyDepartures.slice(0, 3).forEach(e => {
            console.log(
                `  ${e.startDate.toLocaleDateString()} - ${e.durationMinutes}min`
            );
        });
    } else {
    ...
    if (report.records.length > 0) {
        ...
        console.log(
            `  Duration: ${first.durationMinutes}min (${first.durationHours}h)`
        );

...
import { getWeekCode, YpareoClient } from '../src/ypareo';
dotenv.config();
...
client.on('ready', async () => {
    ...
    periods.forEach(period => {
        console.log(
            `- ${period.name} (Session: ${period.sessionCode || 'N/A'})`
        );
        console.log(
            `  Code: ${period.code}, Year: ${period.year || 'N/A'}, Full Year: ${period.isFullYear}`
        );
    });
    console.log();
    ...
    console.log(
        `Overall average: ${fullYear.overallAverage !== null ? fullYear.overallAverage.toFixed(2) : 'N/A'}/20\n`
    );
    ...
    fullYear.subjects.forEach(subject => {
        ...
        console.log(
            `  Student average: ${subject.stats.studentAverage !== null ? subject.stats.studentAverage.toFixed(2) : 'N/A'}/20`
        );
        console.log(
            `  Group average: ${subject.stats.groupAverage !== null ? subject.stats.groupAverage.toFixed(2) : 'N/A'}/20`
        );
        console.log(
            `  Min: ${subject.stats.minAverage !== null ? subject.stats.minAverage.toFixed(2) : 'N/A'} | Max: ${subject.stats.maxAverage !== null ? subject.stats.maxAverage.toFixed(2) : 'N/A'}`
        );

        if (subject.hasComment) console.log(`  Comment: ${subject.comment}`);
        ...
        subject.grades.forEach(grade => {
            const statusEmoji =
                ...
                : '⏳';
            const valueStr =
                grade.isGraded ? `${grade.value}/20`
                : grade.isAbsent ? grade.absenceReason
                : 'Not graded';

            console.log(`    ${statusEmoji} ${grade.theme} - ${valueStr}`);
            console.log(
                `       ${grade.teacher} (${grade.teacherInitials}) | ${grade.date.toLocaleDateString()} | Coeff: ${grade.coefficient} | Type: ${grade.type}`
            );
        });
    });
    console.log();
    ...
    if (sem1) {
        ...
        console.log(
            `Overall average: ${sem1Report.overallAverage !== null ? sem1Report.overallAverage.toFixed(2) : 'N/A'}/20`
        );

        sem1Report.subjects.forEach(subject => {
            if (subject.hasGrades)
                console.log(
                    `  - ${subject.name}: ${subject.stats.studentAverage !== null ? subject.stats.studentAverage.toFixed(2) : 'N/A'}/20 (${subject.grades.length} grades)`
                );
        });
    }
    console.log();

    const englishSubject = fullYear.subjects.find(s =>
        s.name.includes('ANGLAIS')
    );

    if (englishSubject) {
        const englishReport = await client.grades.fetchFullYear(
            regCode,
            englishSubject.code
        );
        ...
        englishReport.subjects.forEach(subject => {
            console.log(
                `${subject.name}: ${subject.stats.studentAverage !== null ? subject.stats.studentAverage.toFixed(2) : 'N/A'}/20`
            );
            subject.grades.forEach(grade => {
                if (grade.isGraded)
                    console.log(
                        `  - ${grade.theme}: ${grade.value}/20 (${grade.date.toLocaleDateString()})`
                    );
            });
        });
    }
    ...
    if (specificSubject)
        console.log(
            `Found: ${specificSubject.name} with ${specificSubject.grades.length} grades`
        );
    console.log();
    ...
    const refreshedReport = await client.grades.refresh(
        regCode,
        periods.find(p => p.isFullYear)!
    );

    console.log('🔄 Refresh Test:');
    console.log(
        `Refreshed report has ${refreshedReport.subjects.length} subjects`
    );
    console.log();

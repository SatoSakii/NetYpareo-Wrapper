...
client.on('ready', async () => {
    ...
    console.log(
        `Contains cookies: ${sessionData.includes('cookie') || sessionData.includes('Cookie')}`
    );
    console.log();
    ...
    console.log(
        `Restored user: ${restoredUser.fullName || restoredUser.username}`
    );
    console.log(
        `Client user: ${client.user?.fullName || client.user?.username}`
    );
    console.log();

    const planning = await client.grades.fetchFullYear(
        client.user!.defaultRegistration!.code
    );

    console.log(
        `Can fetch data: ${planning.subjects.length > 0 ? 'YES' : 'NO'}`
    );
    console.log();

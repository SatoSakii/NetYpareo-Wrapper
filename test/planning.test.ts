import { YpareoClient, getWeekCode, getCurrentWeekCode } from '../src/ypareo';
...
client.on('ready', async () => {
	...
	planning.today.forEach(s => {
		...
		console.log(`    Teacher: ${s.teacher} | Room: ${s.room}${s.hasHomework ? ' ✏️' : ''}`);
	});
	console.log();
	...
	monday.sort((a, b) => a.startMinute - b.startMinute).forEach(s => {
		console.log(`  ${s.startTime}-${s.endTime} ${s.label}`);
	});
	console.log();
	...
	if (planning.sessions.length > 0) {
		...
		console.log(`  Duration: ${firstSession.duration}min (${firstSession.durationHours}h)`);
	...
	console.log(`Week ${planning2.week.weekNumber}: ${planning2.sessions.length} sessions`);
	console.log();
	...
	try {
		...
		console.log(`PDF type: ${pdf instanceof Buffer ? 'Buffer' : typeof pdf}`);
	} catch (err: any) {

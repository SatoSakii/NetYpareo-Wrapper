...
export class Cookie {
		...
		for (const attr of attributes) {
			...
			switch (attrKey) {
				...
				case 'path':
					...
					break;
				case 'expires':
					options.expires = attrValue ? new Date(attrValue) : undefined;
					break;
				case 'max-age':
	...
	matches(urlString: string): boolean {
		try {
			...
			if (this.domain) {
				const cookieDomain = this.domain.startsWith('.') ? this.domain.slice(1) : this.domain;
				const matchesDomain = url.hostname === cookieDomain || url.hostname.endsWith(`.${cookieDomain}`);

				if (!matchesDomain)
					return false;
			}
			if (this.path && !url.pathname.startsWith(this.path))
				return false;
			if (this.secure && url.protocol !== 'https:')
				return false;
			return true;
		} catch {
			return false;
	 ...
	 */
	isExpired(): boolean {
		if (this.value === 'deleted' || this.maxAge === 0)
			return true;

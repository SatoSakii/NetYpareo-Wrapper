...
import { GradeReport, Subject, Grade, SubjectStats, Period } from '../../models';
...
export function parseGrades(html: string): GradeReport {
	...
	const periodName = $('.content-informations-subtitle').text().trim() || 'Current Period';
	const registrationName = periodName.match(/l'inscription (.+)$/)?.[1] || 'Current Registration';
	...
	$('.liste-devoirs-par-matiere').each((_, subjectElem) => {
		...
		const subjectName = $subject.find('.category-header-title').text().trim();

		const grades = parseGradesTable($, $subject);
		const stats = parseStats($, $subject);
		const comment = parseComment($, $subject);

		subjects.push(new Subject(subjectCode, subjectName, grades, stats, comment));
	});
...
function parseGradesTable($: cheerio.CheerioAPI, $subject: cheerio.Cheerio<any>): Grade[] {
	const grades: Grade[] = [];
	...
	$subject.find('tbody.table-body tr').each((_, row) => {
		...
		const themeText = $cells.eq(2).find('.cursor-help').attr('data-tooltip-content')
			|| $cells.eq(2).text().trim();

		const coeffStr = $cells.eq(3).text().trim();
		const coefficient = coeffStr ?  parseFloat(coeffStr.replace(',', '.')) : 1;
		...
		if ($gradeCell.find('span[data-tooltip-content]').length > 0) {
			status = 'absent';
			absenceReason = $gradeCell.find('span').attr('data-tooltip-content') || gradeText;
		} else if (gradeText && ! isNaN(parseFloat(gradeText.replace(',', '.')))) {
...
function parseStats($: cheerio.CheerioAPI, $subject: cheerio.Cheerio<any>): SubjectStats {
	...
	statsTable.each((_, row) => {
		...
		if (label.includes('Moyenne de l\'apprenant'))
			studentAverage = value;
		else if (label.includes('Moyenne du groupe'))
			groupAverage = value;
		else if (label.includes('Moyenne minimale'))
			minAverage = value;
		else if (label.includes('Moyenne maximale'))
			maxAverage = value;
	});

	return new SubjectStats(studentAverage, groupAverage, minAverage, maxAverage);
}

function parseComment($: cheerio.CheerioAPI, $subject: cheerio.Cheerio<any>): string | undefined {
	const commentText = $subject.find('.section-title:contains("Appréciation")')
...
function determineEvaluationType(typeText: string): EvaluationType {
	...
	if (lower.includes('écrit') || lower.includes('ecrit'))
		return 'written';
	if (lower.includes('oral'))
		return 'oral';
	if (lower.includes('pratique') || lower.includes('tp'))
		return 'practical';

 ...
 */
export function parsePlanning(html: string, manager: PlanningManager): Planning {
...
function extractPlanningJSON(html: string): RawPlanningData {
	...
	if (!planningData)
		throw new Error('Planning JSON not found in HTML');

	return planningData;
}

function buildPlanning(raw: RawPlanningData, manager: PlanningManager): Planning {
	...
	const weeks = raw.semaines.map(weekData => {
		const resources = weekData.ressources.map(resourceData => {
			const sessions = resourceData.seances.map(sessionData => {
				const icons = sessionData.icones.map(iconData =>
					new Icon(iconData.libelle, iconData.classe)
				);
			...
			const constraints = resourceData.contraintes.map(constraintData =>
				new Constraint(

...
import type { RecordStatus, RecordType } from '../../models/attendance'
import { AttendRecord, Report, Summary } from '../../models/attendance'
...
function parseSummary($: cheerio.CheerioAPI): Summary {
    ...
    tables
        ...
        .each((_, row) => {
            ...
            if (label.includes('Absences')) absencesMinutes = minutes
            else if (label.includes('Retards')) latesMinutes = minutes
    ...
    tables
        ...
        .each((_, row) => {
            ...
            if (label.includes('Justifiées')) justifiedMinutes = minutes
...
function parseRecords($: cheerio.CheerioAPI): AttendRecord[] {
    ...
    $('.table-striped-decorator tbody tr').each((_, row) => {
        ...
        if (type === 'late') reason = reason.replace(/Absence/g, 'Retard')

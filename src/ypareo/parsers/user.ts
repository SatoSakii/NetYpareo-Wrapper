...
function parseRegistrations($: cheerio.CheerioAPI): Registration[] {
		...
		$(selector).each((_, elem) => {
			...
			if (selector.includes('select')) {
				...
				if (match) {
					...
					name = $elem.closest('.block').find('.block-toolbar h3, .category-header-title').first().text().trim();
				}
			}
			...
			if (code && !seen.has(code)) {
				seen.add(code);
				registrations.push(new Registration(code, name || `Inscription ${code}`, year));
			}
		});
	});
	...
	if (registrations.length === 0) {
		...
		if (match) {
			...
			registrations.push(new Registration(code, `Inscription ${code}`, ''));
		}
	}

...
import type { SessionState, SerializedSession } from '../types';
...
export class SessionManager {
	...
	serialize(): string {
		if (!this.isConnected() || !this.user)
			throw new Error("Cannot serialize session: not connected or user is null.");
	 ...
	 */
	static isSessionValid(data: string, maxAge: number = 24 * 60 * 60 * 1000): boolean {
		try {
			const session: SerializedSession = JSON.parse(data);
